...
const UNLOAD = 0;
const EMPTY = {
    empty: true, //this tile is empty, so empty filed is setted to true;
    needSave: false, //if this is empty tile, then don't need to save to db, so needSave field is setted to false.
    numPoints: 0, //this tile is empty, so it not containt any points
    status: LOADED //this tile is empty, it don't need to saved to db, so status member of this is always setted to LOADED                  
};
...
L.GridLayer.MaskCanvas = L.GridLayer.extend({
    ...
    makeDataPoly: function() {
        ...
        var interval = (dlength / NUMPOLYGON) >> 0; //divide and then floor
    ...
    setData: function(dataset) {
        ...
        //this is global boundary box of all points, so this.BBGlobalLatLng field is redundant
    ...
    getStoreObj: function(id) {
        ...
        //if not found stored object, then reject
        //else if found stored object, thes response this getted object.
        //if object is loaded from the database, we need to mark needSave member of it to fakse, so in later time
        //we know we do not need to backup it again down to db.
        ...
        var promise = new Promise(function(res, rej) {
            ...
            if (db) {
                ...
                db.get(id, {
                    ...
                    //if doc not contain img and doc not containt any points, then 
                    //load img from database, and attach it to doc object
    ...
    getTile: function(coords) {
         ...
         * 1. the first thing is check if tile of coords contain any points by call iscollides functions, if 
         *     this tile not collide then this tile must not containt any points, so we don't need to draw anything in this tile, or
         *     this tile must is emptytile, and then we return promise resolve empty tile.
         *
         * 2. the second thing to do is check if this tile in in lru caching tiles or hugetiles.
         *     if we found tile in cache then we need to set needsave field of this tile base on this status filed of it.
         *     if this status field have been set to loaded, then it have was stored in db in far early, and we no longer 
         *     need to save this tile again, so we set the member needSave of this = false.
         * 3. 
         * 
         */
        ...
        //if not collides, so this tile not containt any points, so we don't need to draw this tile and 
        //return resolve empty tile
        if (!valid) {
        ...
        //tile.status == unload must be set somewhere, so i must find it later to fullly understand this code
        if (!tile || tile.status == UNLOAD) {

            //if tile notfound in memory or tile.status == unloaded
            
            if (tile) {
                console.log(tile.status);
                if (tile.status == UNLOAD) {
                    console.log('unload');
                }
            }
            //
            ...
            //if tile not exist in memory  caching then you need to load it from db. 
            // but this function is asynchrounous, so while the function is excute, very likely that this function 
            // will invoked again. but you just need to load it onetime in that short time, so we set this status 
            // for it is LOADING and then we set it again in lru cache so other promise can see the status of it and do not
            // excute the critical function again
            tile.status = LOADING;
            ...
            /**
             *@[general description]  
             *this promis response for this function: 
             *1.if try to load tile from db
             *2.if tile is founded in db, then it resolve this tile
             *3.else if tile not found in db, in case of error when call asynchonous function getStoreObj then it construct tile 
             *by some calculation in which relate with RTREE and reslove this constructed tile
             */
            ...
            var promise = new Promise(function(resolve, reject) {
                ...
                var out = self.getStoreObj(id).then(function(res) {
                    //if tile founed in db, we need to store tile again. and we set status of it is LOADED.
                    self.store(id, res);
                    res.status = LOADED;
                    ...
                    // console.log(err);
                    // if tile not found in db, then we need to calculate it of it by some calculation relate with RTREE.
                    ...
                    tile = {
                        ...
                        bb: bb,
                        status: LOADED,  //why when construct new tile , the status member of it is set to LOADED
                        needSave: true
                    }
                    ...
                    if (numPoints >= HUGETILE_THREADSHOLD) {
                        ...
                        if (!nTile || nTile.status != LOADED) {
                            self.hugeTiles.set(id, tile);  //dat lai len tren
                            self.tiles.remove(id);
                            resolve(tile);
                        } else resolve(nTile);
    ...
    backupOne: function() {
        ...
        if (db && self.needPersistents > 0) {
            ...
            //why here needPersistents = 0; I need to find out in order to fully understand this code
